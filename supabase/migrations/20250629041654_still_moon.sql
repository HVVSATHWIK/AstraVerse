/*
  # Fix metrics table creation

  1. Changes
    - Add existence checks before creating tables, indexes, policies, and functions
    - Ensure metrics table is only created if it doesn't already exist
    - Ensure indexes are only created if they don't already exist
    - Ensure RLS policies are only created if they don't already exist
    - Ensure function is recreated with OR REPLACE
*/
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'metrics_user_id_metric_name_idx') THEN
    CREATE INDEX metrics_user_id_metric_name_idx ON public.metrics (user_id, metric_name);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'metrics_created_at_idx') THEN
